...
class D4UnwrappingTest {
    ...
    fun `the command comes back out of the packet the generator built`() {
        ...
        val command = nl.rlabs.epsonreset.protocol.EscpRemote.remoteCommandOf(packet)
    ...
    fun `channel packets have nothing to unwrap`() {
        for (packet in SequenceGenerator.handshake() + SequenceGenerator.creditPair()) {
            assertTrue(nl.rlabs.epsonreset.protocol.EscpRemote.isChannelPacket(packet))
            assertNull(nl.rlabs.epsonreset.protocol.EscpRemote.remoteCommandOf(packet))
        }
    }
}
